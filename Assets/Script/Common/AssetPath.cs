...
public class AssetPath
{
    ...
    public static readonly string UpURL = "http://192.168.0.133:8088/assets/upload/";              //资源服务器上传地址
    public static readonly string DownURL = "http://192.168.0.133:8088/assets/download?filename=";       //资源服务器下载地址
    ...
    public static string GetPathHaveBegin(string v)
    {
    ...
    {
        return DownURL + "/" + ChangPathEnd(path).Replace(@"\", @"/");
    }
    ...
    public static void GetFilePathsAndMD5(string fullPath, Dictionary<string, string> versions)
    {
        ...
        foreach (FileInfo info in Dir.GetFiles())
        {
            ...
            string name = GetPathOutBegin(info.FullName, @"Assets\");
            versions.Add(name, GetMD5(info.FullName));
        }
    }
    //获取文件的MD5
